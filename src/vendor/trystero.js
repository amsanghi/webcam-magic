var Trystero = (() => {
  ...
  // ../../../private/tmp/claude-501/-Users-amansanghi-Downloads/a7aa22a8-6014-4f80-aa9e-449fd6e7a567/scratchpad/wtbuild/entry.js
  var entry_exports = {};
  __export(entry_exports, {
    mqtt: () => dist_exports2,
    selfId: () => selfId,
    torrent: () => dist_exports
  });

  // ../../../private/tmp/claude-501/-Users-amansanghi-Downloads/a7aa22a8-6014-4f80-aa9e-449fd6e7a567/scratchpad/wtbuild/node_modules/@trystero-p2p/torrent/dist/index.mjs
  var dist_exports = {};
  ...
  // ../../../private/tmp/claude-501/-Users-amansanghi-Downloads/a7aa22a8-6014-4f80-aa9e-449fd6e7a567/scratchpad/wtbuild/node_modules/@trystero-p2p/core/dist/utils.mjs
  ...
  var shuffle = (xs, seed) => {
    const a = [...xs];
    const rand = () => {
      const x2 = sin(seed++) * 1e4;
      return x2 - floor(x2);
    };
    let i = a.length;
    while (i) {
      const j2 = floor(rand() * i--);
      const tmp = a[i];
      a[i] = a[j2];
      a[j2] = tmp;
    }
  ...
  // ../../../private/tmp/claude-501/-Users-amansanghi-Downloads/a7aa22a8-6014-4f80-aa9e-449fd6e7a567/scratchpad/wtbuild/node_modules/@trystero-p2p/core/dist/crypto.mjs
  var algo = "AES-GCM";
  var strToSha1 = {};
  ...
  // ../../../private/tmp/claude-501/-Users-amansanghi-Downloads/a7aa22a8-6014-4f80-aa9e-449fd6e7a567/scratchpad/wtbuild/node_modules/@trystero-p2p/core/dist/offer-pool.mjs
  var offerTtl = 57333;
  ...
  // ../../../private/tmp/claude-501/-Users-amansanghi-Downloads/a7aa22a8-6014-4f80-aa9e-449fd6e7a567/scratchpad/wtbuild/node_modules/@trystero-p2p/core/dist/handshake.mjs
  ...
  // ../../../private/tmp/claude-501/-Users-amansanghi-Downloads/a7aa22a8-6014-4f80-aa9e-449fd6e7a567/scratchpad/wtbuild/node_modules/@trystero-p2p/core/dist/peer.mjs
  var iceTimeout = 15e3;
  ...
  var peer_default = (initiator, { trickleIce, rtcConfig, rtcPolyfill, turnConfig, _test_only_mdnsHostFallbackToLoopback }) => {
    const pc2 = new (rtcPolyfill ?? RTCPeerConnection)({
    ...
    const getRemoteUfrag = () => {
      const sdp = pc2.remoteDescription?.sdp;
      if (!sdp) return null;
      return sdp.match(/a=ice-ufrag:([^\s]+)/)?.[1] ?? null;
    };
    const getRemoteMediaSectionCount = () => (pc2.remoteDescription?.sdp?.match(/^m=/gm) ?? []).length;
    const canApplyRemoteCandidate = (candidate) => {
      if (!pc2.remoteDescription) return false;
    ...
    const flushPendingRemoteCandidates = async () => {
      if (!pc2.remoteDescription || pendingRemoteCandidates.length === 0) return;
    ...
    const emitLocalDescriptionSignal = async () => {
      const signal = shouldTrickleIce ? localDescriptionSignal(pc2) : await waitForIceGathering(pc2);
      emitSignal(signal);
      return signal;
    };
    if (initiator) {
      dataChannel = pc2.createDataChannel("data");
      setupDataChannel(dataChannel);
    } else pc2.ondatachannel = ({ channel }) => {
      dataChannel = channel;
      setupDataChannel(channel);
    };
    const createOffer = async (restartIce = false) => {
      if (pc2.connectionState === "closed") return;
      try {
        makingOffer = true;
        if (restartIce) {
          if (pc2.signalingState !== "stable" && pc2.signalingState !== "closed" && pc2.localDescription?.type === offerType) await pc2.setLocalDescription({ type: "rollback" });
          if (typeof pc2.restartIce === "function") pc2.restartIce();
        }
        await pc2.setLocalDescription(restartIce ? await pc2.createOffer({ iceRestart: true }) : void 0);
    ...
    pc2.onnegotiationneeded = async () => createOffer(false);
    ...
    const handleConnectionStateChange = () => {
      if (pc2.connectionState === "failed" || pc2.connectionState === "closed" || pc2.iceConnectionState === "failed" || pc2.iceConnectionState === "closed") {
        emitClose();
        return;
      }
      if (pc2.connectionState === "connected" || pc2.connectionState === "connecting" || pc2.iceConnectionState === "connected" || pc2.iceConnectionState === "completed" || pc2.iceConnectionState === "checking") {
        clearDisconnectedCloseTimer();
        return;
      }
      if (pc2.connectionState === "disconnected" || pc2.iceConnectionState === "disconnected") {
        if (!disconnectedCloseTimer) disconnectedCloseTimer = setTimeout(() => {
          disconnectedCloseTimer = null;
          if (pc2.connectionState === "disconnected" || pc2.iceConnectionState === "disconnected") emitClose();
        }, disconnectedCloseDelayMs);
        return;
      }
    };
    pc2.onconnectionstatechange = handleConnectionStateChange;
    pc2.addEventListener(iceConnectionStateEvent, handleConnectionStateChange);
    pc2.ontrack = (e) => {
    ...
    pc2.onremovestream = (e) => handlers.stream?.(e.stream);
    ...
    if (initiator) queueMicrotask(() => {
      if (!makingOffer && pc2.signalingState === "stable" && !pc2.localDescription && pc2.connectionState !== "closed") pc2.onnegotiationneeded?.(new Event("negotiationneeded"));
    });
    return {
      created: Date.now(),
      connection: pc2,
      ...
      get isDead() {
        return pc2.connectionState === "closed";
      },
      getOffer: async (restartIce = false) => {
        if (!initiator) return;
        if (restartIce) return createOffer(true);
        if (pc2.localDescription?.type === offerType) return shouldTrickleIce ? localDescriptionSignal(pc2) : waitForIceGathering(pc2);
        return offerPromise;
      },
      async signal(sdp) {
        ...
        try {
          ...
          if (sdp.type === offerType) {
            if (makingOffer || pc2.signalingState !== "stable" && !isSettingRemoteAnswerPending) {
              if (initiator) return;
              await all([pc2.setLocalDescription({ type: "rollback" }), pc2.setRemoteDescription(rtcSdp)]);
            } else await pc2.setRemoteDescription(rtcSdp);
          ...
          if (sdp.type === answerType) {
            ...
            try {
              await pc2.setRemoteDescription(rtcSdp);
              await flushPendingRemoteCandidates();
            } finally {
      ...
      offerPromise,
      addStream: (stream) => stream.getTracks().forEach((track) => pc2.addTrack(track, stream)),
      removeStream: (stream) => pc2.getSenders().filter((sender) => sender.track && stream.getTracks().includes(sender.track)).forEach((sender) => pc2.removeTrack(sender)),
      addTrack: (track, stream) => pc2.addTrack(track, stream),
      removeTrack: (track) => {
        const sender = pc2.getSenders().find((s) => s.track === track);
        if (sender) pc2.removeTrack(sender);
      },
      replaceTrack: (oldTrack, newTrack) => {
        const sender = pc2.getSenders().find((s) => s.track === oldTrack);
  ...
  // ../../../private/tmp/claude-501/-Users-amansanghi-Downloads/a7aa22a8-6014-4f80-aa9e-449fd6e7a567/scratchpad/wtbuild/node_modules/@trystero-p2p/core/dist/action-wire.mjs
  ...
  // ../../../private/tmp/claude-501/-Users-amansanghi-Downloads/a7aa22a8-6014-4f80-aa9e-449fd6e7a567/scratchpad/wtbuild/node_modules/@trystero-p2p/core/dist/actions.mjs
  ...
  // ../../../private/tmp/claude-501/-Users-amansanghi-Downloads/a7aa22a8-6014-4f80-aa9e-449fd6e7a567/scratchpad/wtbuild/node_modules/@trystero-p2p/core/dist/media.mjs
  ...
  // ../../../private/tmp/claude-501/-Users-amansanghi-Downloads/a7aa22a8-6014-4f80-aa9e-449fd6e7a567/scratchpad/wtbuild/node_modules/@trystero-p2p/core/dist/room.mjs
  ...
  var room_default = (onPeer, onPeerLeave, onSelfLeave, { onPeerHandshake, onHandshakeError, handshakeTimeoutMs = defaultHandshakeTimeoutMs, isPassive = false } = {}) => {
    ...
    onPeer((peer, id) => {
      ...
      peer.setHandlers({
        ...
        error: (err) => {
          console.debug(`${libName} peer error:`, err);
  ...
  // ../../../private/tmp/claude-501/-Users-amansanghi-Downloads/a7aa22a8-6014-4f80-aa9e-449fd6e7a567/scratchpad/wtbuild/node_modules/@trystero-p2p/core/dist/shared-peer.mjs
  ...
  // ../../../private/tmp/claude-501/-Users-amansanghi-Downloads/a7aa22a8-6014-4f80-aa9e-449fd6e7a567/scratchpad/wtbuild/node_modules/@trystero-p2p/core/dist/signal-handler.mjs
  ...
  // ../../../private/tmp/claude-501/-Users-amansanghi-Downloads/a7aa22a8-6014-4f80-aa9e-449fd6e7a567/scratchpad/wtbuild/node_modules/@trystero-p2p/core/dist/strategy.mjs
  ...
  // ../../../private/tmp/claude-501/-Users-amansanghi-Downloads/a7aa22a8-6014-4f80-aa9e-449fd6e7a567/scratchpad/wtbuild/node_modules/@trystero-p2p/core/dist/topic-strategy.mjs
  var signalKeys2 = [
    "offer",
    "answer",
    "candidate"
  ...
  // ../../../private/tmp/claude-501/-Users-amansanghi-Downloads/a7aa22a8-6014-4f80-aa9e-449fd6e7a567/scratchpad/wtbuild/node_modules/@trystero-p2p/mqtt/dist/index.mjs
  var dist_exports2 = {};
  __export(dist_exports2, {
    defaultRelayUrls: () => defaultRelayUrls2,
    getRelaySockets: () => getRelaySockets2,
    joinRoom: () => joinRoom2,
    selfId: () => selfId
  });

  // ../../../private/tmp/claude-501/-Users-amansanghi-Downloads/a7aa22a8-6014-4f80-aa9e-449fd6e7a567/scratchpad/wtbuild/node_modules/mqtt/dist/mqtt.esm.js
...
`;
    ...
    Uh.exports = class {
      ...
      *[w_]() {
      ...
      [/* @__PURE__ */ Symbol.for("nodejs.util.inspect.custom")](e, r) {
        return __(this, { ...r, depth: 0, customInspect: false });
      }
    };
  });
  var Cn = j((hk, $h) => {
  ...
  var Us = j((Fk, Zh) => {
    ...
    function V_(t, e, r) {
      let n;
      if (typeof e == "string" || e instanceof W_) return new t({ objectMode: true, ...r, read() {
      ...
      let s = new t({ objectMode: true, highWaterMark: 1, ...r }), o = false;
      s._read = function() {
        o || (o = true, c());
  ...
  var Nn = j((Jk, bd) => {
    ...
    le.wrap = function(t, e) {
      var r, n;
      return new le({ objectMode: (r = (n = t.readableObjectMode) !== null && n !== void 0 ? n : t.objectMode) !== null && r !== void 0 ? r : true, ...e, destroy(i, s) {
  ...
  var oa = j((LC, Qd) => {
    ...
    function jt(t) {
      ...
      e === 0 && (t = { ...t, highWaterMark: null, readableHighWaterMark: e, writableHighWaterMark: t.writableHighWaterMark || 0 }), ia.call(this, t), this._readableState.sync = false, this[Dn] = null, t && (typeof t.transform == "function" && (this._transform = t.transform), typeof t.flush == "function" && (this._flush = t.flush)), this.on("prefinish", w0);
    }
    function na(t) {
  ...
  var Ua = j((La) => {
    ...
    var xE = or(), TE = { objectMode: true }, PE = { clean: true }, qa = class {
      ...
      _inflights;
      constructor(e) {
        this.options = e || {}, this.options = { ...PE, ...e }, this._inflights = /* @__PURE__ */ new Map();
      }
      put(e, r) {
    ...
    var NE = (t) => !my.has(t), yL = /* @__PURE__ */ Symbol("type"), lr = (t) => t && t === Math.floor(t) && t > 0 && isFinite(t), vy = (t) => lr(t) ? t <= Math.pow(2, 8) ? Uint8Array : t <= Math.pow(2, 16) ? Uint16Array : t <= Math.pow(2, 32) ? Uint32Array : t <= Number.MAX_SAFE_INTEGER ? bn : null : null, bn = class extends Array {
      ...
      *rvalues() {
      ...
      #M(e, r, n, i) {
        ...
        return r === void 0 ? (this.set(e, P, { ...c.options, status: void 0 }), r = this.#n.get(e)) : this.#e[r] = P, P;
      }
      #t(e) {
      ...
      memo(e, r = {}) {
        ...
        let { context: i, forceRefresh: s, ...o } = r, a = this.get(e, o);
  ...
  var zo = j((yt) => {
    ...
    var Fu = fr(sy()), $A = or(), kg = fr(cy()), HA = fr(At()), Cg = Ng(Na()), vn = fr(Ua()), VA = fr(Ky()), GA = fr(Iu()), zA = fr(Qy()), bi = Mr(), KA = Yy(), QA = fr(ju()), Bg = Ng(yi()), Wu = globalThis.setImmediate || ((...t) => {
      ...
      connect() {
        ...
        if (this.options.will && (c.will = { ...this.options.will, payload: this.options.will?.payload }), this.topicAliasRecv && (c.properties || (c.properties = {}), this.topicAliasRecv && (c.properties.topicAliasMaximum = this.topicAliasRecv.max)), this._writePacket(c), r.on("error", this.emit.bind(this, "error")), this.options.properties) {
      ...
      publish(e, r, n, i) {
        ...
        let { options: s } = this;
        typeof n == "function" && (i = n, n = null), n = n || {}, n = { ...{ qos: 0, retain: false, dup: false }, ...n };
      ...
      subscribe(e, r, n) {
        ...
        let c = { qos: 0 };
        i === 5 && (c.nl = false, c.rap = false, c.rh = 0), r = { ...c, ...r };
...
`) && (e = e.replace(Kx, "%0A")), e.includes("\r") && (e = e.replace(Qx, "%0D")), e.includes("	") && (e = e.replace(Yx, "%09")), n.pathname = e, n;
  ...
  var $l = j((Si) => {
    ...
    function _T(t) {
      let { host: e, port: r, socksProxy: n, ...i } = t;
      if (n !== void 0) {
        let s = (0, wT.default)(e, r, n, { timeout: t.socksTimeout });
        return (0, Lb.connect)({ ...i, socket: s });
      }
  ...
  var $b = j((In) => {
    ...
    function Wb(t, e) {
      if (Fb("connecting to an MQTT broker..."), typeof t == "object" && !e && (e = t, t = ""), e = e || {}, t && typeof t == "string") {
        ...
        if (i.port != null && (s.port = Number(i.port)), s.host = i.hostname, s.query = i.query, s.auth = i.auth, s.protocol = i.protocol, s.path = i.path, e = { ...s, ...e }, !e.protocol) throw new Error("Missing protocol");
  ...
  // ../../../private/tmp/claude-501/-Users-amansanghi-Downloads/a7aa22a8-6014-4f80-aa9e-449fd6e7a567/scratchpad/wtbuild/node_modules/@trystero-p2p/mqtt/dist/index.mjs
  var defaultRedundancy2 = 4;
  var relayManager2 = createRelayManager((client) => client.stream?.socket);
  var msgHandlers2 = relayManager2.scoped();
  var subscriptionTokens2 = relayManager2.scoped();
  var subscriptionRefs = relayManager2.scoped();
  var joinRoom2 = topic_strategy_default({
    init: (config) => getRelays(config, defaultRelayUrls2, defaultRedundancy2).map((url) => {
      const client = relayManager2.register(url, () => mqtt_esm_default.connect(url));
      const handlers = msgHandlers2.forRelay(client);
    ...
    subscribeTopic: (client, topic, onMessage) => {
      const handlers = msgHandlers2.forRelay(client);
      const tokens = subscriptionTokens2.forRelay(client);
      const refs = subscriptionRefs.forRelay(client);
      const token = Symbol(topic);
      const topicHandler = (topic2, data) => onMessage(topic2, data);
